---
title: "Comparing the density of mobile phone cell towers with population: Analysis"
...
LA$res_pop_magn = LA$population_2015/magnitude
LA$NP = LA$NUMPOINTS/magnitude
ggplot(LA, aes(NP, res_pop_magn)) + geom_point(size = 2, alpha = 0.8, stroke= 0) + 
  stat_smooth(method = 'loess') +
  xlab("LAD Cell Towers count in '000s") +
  ylab("LAD UK mainland Residential Population in '000s") +
  geom_text(data=subset(LA, res_pop_magn > 400 | NP > 10 | lad2014_name == "City of London"),
            aes(NP, res_pop_magn,label=lad2014_name), check_overlap = TRUE, hjust=-0.1, vjust=0, size = 4)
```

```{r}
round(cor(LA$NUMPOINTS, LA$population_2015), 2)
```
...
```{r}
LA['x_1000_res'] = LA['NUMPOINTS']/(LA['population_2015']/1000.0)

ggplot(LA, aes(x_1000_res, y=..density..)) + 
...
```{r}
London_LA = filter(LA, grepl('E090000', lad15cd))
head(London_LA)
```
```{r}
cor(London_LA$NUMPOINTS, London_LA$population_2015) # 0.371217
...
MSOA_EW = read.csv("MSOA_EW_wd_res_ct.csv", header = T, sep = ",", stringsAsFactors = F)
MSOA_EW$NP = MSOA_EW$NUMPOINTS/magnitude
MSOA_EW$wd_pop_magn = MSOA_EW$wd_pop/magnitude
MSOA_EW$res_magn = MSOA_EW$mid_2015_res_pop/magnitude
```
...
```{r}
# Number of cell towers per MSOA vs. number of residents per MSOA. England and Wales
ggplot(MSOA_EW, aes(NP, res_magn)) + geom_point(size = 2, alpha = 0.8, stroke= 0) + stat_smooth(method = "loess") +
  xlab("MSOA Cell Towers count in '000s") +
  ylab("MSOA E&W Residential Population in '000s") +
  geom_text(data=subset(MSOA_EW, res_magn > 15 | (NP > 1.5 & NP < 4)),
            aes(NP, res_magn,label=MSOA11NM), check_overlap = TRUE, hjust=-0.06, vjust=0.2, size = 4)+
  geom_text(data=subset(MSOA_EW, NP > 4),
            aes(NP, res_magn,label=MSOA11NM), check_overlap = TRUE,hjust=0.8, vjust=1.2, size = 4)+
  geom_text(data=subset(MSOA_EW, res_magn < 3),
            aes(NP, res_magn,label=MSOA11NM), check_overlap = TRUE,hjust=-0.08, vjust=0.2, size = 4)

```

```{r}
paste("Correlation between cell towers per MSOA vs. number of residents per MSOA")
cor(MSOA_EW$NUMPOINTS, MSOA_EW$mid_2015_res_pop)
```
...
ggplot(MSOA_EW, aes(NP, wd_pop_magn)) + geom_point(size = 2, alpha = 0.8, stroke= 0) + stat_smooth(method = "loess") +
  xlab("MSOA Cell Towers count in '000s") +
  ylab("MSOA E&W Workday Population in '000s") +
  geom_text(data=subset(MSOA_EW, (wd_pop_magn > 80 & wd_pop_magn < 300) | (NP > 1.5 & NP < 4)),
            aes(NP, wd_pop_magn,label=MSOA11NM), check_overlap = TRUE, hjust=-0.06, vjust=0.2, size = 4)+
  geom_text(data=subset(MSOA_EW, NP > 4),
            aes(NP, wd_pop_magn,label=MSOA11NM), check_overlap = TRUE,hjust=1.1, size = 4)
```

```{r}
cor(MSOA_EW$NUMPOINTS, MSOA_EW$wd_pop) 
```
...
```{r}
# Number of cell towers per MSOA vs number of workday population per MSOA. London only
MSOA_London = read.csv("MSOA_London_wd_res_ct.csv", header = T, sep = ",", stringsAsFactors = F)
```

```{r}
MSOA_London$wd_pop_mag = MSOA_London$wd_pop/magnitude
MSOA_London$NP = MSOA_London$NUMPOINTS/magnitude

ggplot(MSOA_London, aes(NP, wd_pop_mag)) + geom_point(size = 2, alpha = 0.8, stroke= 0) + stat_smooth(method = "loess") +
  xlab("MSOA London Cell Towers count in '000s") +
  ylab("MSOA London Workday Population in '000s") +
  geom_text(data=subset(MSOA_London, (wd_pop_mag > 100 & wd_pop_mag < 300) | (NP > 1 & NP < 4)),
            aes(NP, wd_pop_mag,label=MSOA11NM), check_overlap = TRUE, , hjust=-0.06, vjust=0.2, size = 4) +
  geom_text(data=subset(MSOA_London, NP > 4),
            aes(NP, wd_pop_mag,label=MSOA11NM), check_overlap = TRUE,hjust=1.1, size = 4)
```

```{r}
cor(MSOA_London$NUMPOINTS, MSOA_London$mid_2015_res_pop) # 0.1461389
cor(MSOA_London$NUMPOINTS, MSOA_London$wd_pop) # 0.8581741
```
