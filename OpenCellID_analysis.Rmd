---
title: 'Part 1: Comparing the density of mobile phone cell towers with population:
  Analysis'
output:
...
```{r}
paste0("Corraletion between cell tower counts and Resiential population at MSOA Level (England & Wales): ", round(cor(MSOA_EW$NP, MSOA_EW$res),2))
```
...
```{r}
paste0("Corraletion between cell tower counts and WorkDay population at Local Authority District Level (London only): ", round(cor(MSOA_EW$NP, MSOA_EW$wd), 2)) # 0.51
```
...
```{r}
paste0("Corraletion between cell tower counts and Resiential population at MSOA Level (London only): ", round(cor(MSOA_London$NP, MSOA_London$res), 2)) # 0.15

paste0("Corraletion between cell tower counts and WorkDay population at MSOA Level (London only): ", round(cor(MSOA_London$NP, MSOA_London$wd), 2)) # 0.86
```
...
```{r}
paste0("Corraletion between cell tower counts and Resiential population at LSOA Level (London only): ", round(cor(LSOA_London$NP, LSOA_London$res), 2)) # 0.14

paste0("Corraletion between cell tower counts and WorkDay population at LSOA Level (London only): ", round(cor(LSOA_London$NP, LSOA_London$wd), 2)) # 0.82
```
